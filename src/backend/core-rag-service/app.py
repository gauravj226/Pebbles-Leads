...
def test_index():
    """Get the first 5 entries in the index"""
    result = processor.get_first_five_index_entries()
    return jsonify(result)
    
@app.route('/test/retrieval', methods=['GET'])
def test_retrieval():
    """Get the first 5 entries in the index"""
    result = processor.retrieve_documents(query="Mental Health", query_type="factual")
    return jsonify(result)
    
@app.route('/test/process_query', methods=['GET'])
def test_process_query_retrieval():
    """Test query processing with retrieval and return JSON response"""
    try:
        # Simulate a JSON payload for testing
        test_data = {
            "query": "Mental Health",
            "session_id": str(uuid.uuid4())
        }
        
        # Create a mock request context to pass to process_query
        with app.test_request_context(json=test_data):
            result = process_query(test_type="retrieval")
            return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
